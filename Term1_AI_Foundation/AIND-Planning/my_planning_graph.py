...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Iterate throught possible actions for the problem
        for action in self.all_actions:
            # create a node to add to the action level
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level-1]:
            for node_s in node_a.effnodes: # node_a.effnodes denote the result of action which is a literal
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Check for inconsistency effects between node_a1 and node_a2
        # for every action of one node , we need to make sure effect_add and effect_rem
        # are not effect_rem and effect_add of another node respectively.
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # effect of one action is a negation of a precondition of another action.
        for eff_add in node_a1.action.effect_add:
            for precon_neg in node_a2.action.precond_neg:
                if eff_add == precon_neg:
                    return True
        for eff_rem in node_a1.action.effect_rem:
            for precon_pos in node_a2.action.precond_pos:
                if eff_rem == precon_pos:
                    return True

        for eff_add in node_a2.action.effect_add:
            for precon_neg in node_a1.action.precond_neg:
                if eff_add == precon_neg:
                    return True
        for eff_rem in node_a2.action.effect_rem:
            for precon_pos in node_a1.action.precond_pos:
                if eff_rem == precon_pos:
                    return True
        return False
    ...
    def level_cost(self, goal):
        """
        Helper function for h_levelsum to calculate the level cost for each goal in a planning
        graph

        :param goal: the goal node to be found in a planning graph, at the literal levels
        :return: level_cost: the number of levels we had to move through a planning graph to find the
                            goal.
        """
        for level_counter, s_level in enumerate(self.s_levels):
